...
@Injectable()
export class CycleTimeQuery {
  constructor(@InjectDataSource() private readonly ds: DataSource) {}

  async execute(input: CycleTimeInput): Promise<DurationStatsDto[]> {
    const { workspaceId, period, from, to, projectId } = input;
    const durExpr = `EXTRACT(EPOCH FROM (t.completed_at - fip.started_at))::float`;
    const rows = await this.ds
      ...
      .from('tasks', 't')
      .innerJoin(
        `(SELECT al.subject_id AS task_id, MIN(al.occurred_at) AS started_at
          FROM audit_logs al
          WHERE al.workspace_id = :workspaceId
            AND al.action = 'TASK_STATUS_CHANGED'
            AND al.diff->>'newCategory' <> 'todo'
